...
class Aggregator:
    ...
    def generate_score(self, entry):
        """
            Calculate a rank for a passed entry, using date and rank.
    ...
    def feed_to_object(self, parsed_feed, count=10):
        """Normalize feedparser object.
          Args:
            parsed_feed (feedparser.FeedParserDict): Parsed feed, returned from feedparser.
            count (integer, optional): Number of entries to pull.
        """
        return {
    ...
    def get_thumbnail(self, entry):
        """Given an RSS entry, extract a thumbnail if one exists.
          Args:
    ...
    def get_feed_logo(self, feed):
        """Given a feed, extract a logo image url.
          Args:
        ...
        if _logo not in self.cached_urls.keys():
            try:
                self.cached_urls[_logo] = requests.get(
                    _logo).status_code in [200, 301, 302]
            except Exception:
...
class DatabaseAggregator(NewsAggregator):
    """NewsAggregator child class, to handle database. 
    """
